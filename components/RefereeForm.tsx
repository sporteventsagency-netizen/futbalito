...
interface RefereeFormProps {
  ...
  onSave: (data: Omit<Referee, 'id' | 'photoUrl'> & { photoFile?: File | null }) => void;
  onClose: () => void;
}
...
const RefereeForm: React.FC<RefereeFormProps> = ({ referee, onSave, onClose }) => {
  ...
  useEffect(() => {
    if (referee) {
        ...
        setCounty(referee.county || '');
        setCity(referee.city || '');
        setDateOfBirth(referee.dateOfBirth ? new Date(referee.dateOfBirth).toISOString().slice(0,10) : '');
        setPhone(referee.phone || '');
        setEmail(referee.email || '');
        setCategory(referee.category || 'ASPIRANTI');
        setPreviewUrl(referee.photoUrl || null);
    } else {
      setName('');
      setCounty('');
      setCity('');
      ...
      <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
        <div className="flex items-center space-x-4">
            <div>
                ...
                <img src={previewUrl || `https://avatar.iran.liara.run/username?username=${name.replace(/\s/g, '+')}`} alt="Referee" className="mt-1 h-20 w-20 rounded-full object-cover bg-gray-100" />
            </div>
        ...
        <div>
            ...
            <select id="category" value={category} onChange={e => setCategory(e.target.value as RefereeCategory)} className="mt-1 block w-full border rounded-md p-2">
                {refereeCategories.map(c => <option key={c} value={c}>{c.replace('_', ' ')}</option>)}
            </select>
        </div>
